...
// ─── Number Input (مستقل تماماً — مش بيخسر focus أبداً) ──────────────────────
function NumberInput({ value, onCommit, style, placeholder }: {
  value: number; onCommit: (v: number) => void; style?: CSSProperties; placeholder?: string;
}) {
  const [local, setLocal] = useState(value === 0 ? "" : String(value));
  const prev = useRef(value);
  useEffect(() => {
    if (value !== prev.current) { prev.current = value; setLocal(value === 0 ? "" : String(value)); }
  }, [value]);
  const commit = useCallback(() => {
    const n = Math.max(0, Number(local) || 0); prev.current = n; onCommit(n);
  }, [local, onCommit]);
  return (
...
const ALL_MONTHS = ["January 2026","February 2026","March 2026","April 2026","May 2026","June 2026","July 2026","August 2026","September 2026","October 2026","November 2026","December 2026"];
...
const inputStyle = (ok: boolean, blue?: boolean): CSSProperties => ({ width: "100%", padding: "10px 12px", borderRadius: "10px", fontSize: "14px", border: "1.5px solid " + (ok ? (blue ? "#BFDBFE" : "#DDD6FE") : "#FECACA"), backgroundColor: ok ? "#fff" : "#FFF5F5", boxSizing: "border-box" });

// ─── RowLabel ─────────────────────────────────────────────────────────────────
function RowLabel({ label, target, rem, blue }: { label: string; target?: number; rem: number; blue?: boolean }) {
  return (
...
// ─── Dashboard (خارج BudgetApp) ───────────────────────────────────────────────
function Dashboard({ pct, health, totalSpent, extraCash, F_GOLD, birthdayTotal, goldGrams, goldP, setGoldP, SALARY, fmt }: any) {
  return (
...
// ─── Tracker (خارج BudgetApp) ─────────────────────────────────────────────────
function Tracker({ actual, checks, toggleCheck, commitField, T_OUTINGS, T_SHOPPING, T_LASER, T_GYM, T_FOOD, T_TRANSPORT }: any) {
  ...
  return (
    <div style={{ padding: "14px" }}>
      ...
      <div style={{ ...card("#F5F3FF", "#DDD6FE") }}>
        ...
        {[
          { k: "outings", label: "Outings and Trips", target: T_OUTINGS, rem: remOutings },
          { k: "shopping", label: "Shopping", target: T_SHOPPING, rem: remShopping },
          { k: "laser", label: "Laser Session", target: T_LASER, rem: remLaser },
          { k: "gym", label: "Gym Subscription", target: T_GYM, rem: remGym },
        ].map(f => (
          <div key={f.k} style={{ marginBottom: "12px" }}>
            <RowLabel label={f.label} target={f.target} rem={f.rem} />
            <NumberInput value={actual[f.k]} onCommit={v => commitField(f.k, v)} placeholder="Enter amount" style={inputStyle(f.rem >= 0, false)} />
          </div>
        ))}
      </div>
      <div style={{ ...card("#EFF6FF", "#BFDBFE") }}>
        ...
        {[
          { k: "food", label: "Supermarket Food", target: T_FOOD, rem: remFood },
          { k: "transport", label: "Transport and Travel", target: T_TRANSPORT, rem: remTransport },
        ].map(f => (
          <div key={f.k} style={{ marginBottom: "12px" }}>
            <RowLabel label={f.label} target={f.target} rem={f.rem} blue />
            <NumberInput value={actual[f.k]} onCommit={v => commitField(f.k, v)} placeholder="Enter amount" style={{ ...inputStyle(f.rem >= 0, true), borderColor: f.rem >= 0 ? "#BFDBFE" : "#FECACA" }} />
          </div>
        ))}
        <div>
...
// ─── Birthdays (خارج BudgetApp) ───────────────────────────────────────────────
function Birthdays({ birthdayTotal, surpOutings, surpShopping, surpTransport }: any) {
...
// ─── Ledger (خارج BudgetApp) ──────────────────────────────────────────────────
function Ledger({ history, logMonth, setLogMonth, saveMonth }: any) {
  return (
    <div style={{ padding: "14px" }}>
      <div style={{ ...card() }}>
...
export default function BudgetApp() {
  ...
  const remOutings = T_OUTINGS - actual.outings, remShopping = T_SHOPPING - actual.shopping, remTransport = T_TRANSPORT - actual.transport;
  ...
  const commitField = useCallback((field: string, v: number) => setActual((p: any) => ({ ...p, [field]: v })), []);
  ...
  return (
    <div style={{ backgroundColor: "#FDFBF7", minHeight: "100vh", maxWidth: "430px", margin: "0 auto", fontFamily: "'Segoe UI',system-ui,sans-serif", paddingBottom: "80px", position: "relative" }}>
      ...
      {tab === "dashboard" && <Dashboard pct={pct} health={health} totalSpent={totalSpent} extraCash={extraCash} F_GOLD={F_GOLD} birthdayTotal={birthdayTotal} goldGrams={goldGrams} goldP={goldP} setGoldP={setGoldP} SALARY={SALARY} fmt={fmt} />}
      {tab === "tracker"   && <Tracker actual={actual} checks={checks} toggleCheck={toggleCheck} commitField={commitField} T_OUTINGS={T_OUTINGS} T_SHOPPING={T_SHOPPING} T_LASER={T_LASER} T_GYM={T_GYM} T_FOOD={T_FOOD} T_TRANSPORT={T_TRANSPORT} />}
      {tab === "birthdays" && <Birthdays birthdayTotal={birthdayTotal} surpOutings={surpOutings} surpShopping={surpShopping} surpTransport={surpTransport} />}
      {tab === "ledger"    && <Ledger history={history} logMonth={logMonth} setLogMonth={setLogMonth} saveMonth={saveMonth} />}
